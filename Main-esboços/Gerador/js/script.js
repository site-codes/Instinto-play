...
// REDIRECIONAR PAGINA
fetch('https://instinto-play.web.app/Main/Hash/permitidos.json')
  .then(response => response.json())
  .then(data => {
    const urlsPermitidas = data.permitidos; // Array com URLs permitidas
    const urlAtual = window.location.href;
    
    // Verifica se a URL atual está entre as permitidas
    const acessoPermitido = urlsPermitidas.some(url => urlAtual.startsWith(url));

    // Redireciona se a URL atual não estiver entre as permitidas
    if (!acessoPermitido) {
      window.location.href = "https://instinto-play.web.app/Main/acessNegado.html";
    }
  })
  .catch(error => {
    console.error("Erro ao carregar o JSON:", error);
    // Opcional: redireciona em caso de erro na carga do JSON
    window.location.href = "https://instinto-play.web.app/Main/acessNegado.htm";
  });
